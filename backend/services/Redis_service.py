...
class RedisSessionManager:
    ...
    async def close(self):
        ...
        # }
    async def create_session(self, user_id: str, task_id: str, session_id: Optional[str] = None,
                            status: str = "active",
    ...
    async def set_task_status(self, task_id: str, status: str, result: Optional[Dict] = None,
        ...
        await self.redis_client.set(
            ...
            ex=app_config.TASK_TTL
        )
        # 如果提供了 user_id 和 session_id，将 task_id 绑定到用户和会话
        if user_id and session_id:
            await self.redis_client.sadd(f"task_mapping:{user_id}:{session_id}", task_id)
            await self.redis_client.expire(f"task_mapping:{user_id}:{session_id}", app_config.TASK_TTL)
